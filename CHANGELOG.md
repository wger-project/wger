...
**Unified JWT signing keys.**  The `SIGNING_KEY` environment variable is gone:
replace it with `JWT_PRIVATE_KEY` and `JWT_PUBLIC_KEY` generated by
`docker compose exec web ./manage.py generate-jwt-keys`. Note that existing
tokens will become invalid and logged-in users will need to login again.
