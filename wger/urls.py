...
router.register(
    r'public-templates', manager_api_views.PublicRoutineTemplateViewSet, basename='public-templates'
)
router.register(
...
router.register(
    r'max-weight-config', manager_api_views.MaxWeightConfigViewSet, basename='max-weight-config'
)
router.register(
    r'repetitions-config', manager_api_views.RepetitionsConfigViewSet, basename='repetitions-config'
)
router.register(
...
router.register(
    r'max-sets-config', manager_api_views.MaxSetsConfigViewSet, basename='max-sets-config'
)
router.register(r'rest-config', manager_api_views.RestConfigViewSet, basename='rest-config')
router.register(
    r'max-rest-config', manager_api_views.MaxRestConfigViewSet, basename='max-rest-config'
)
...
router.register(
    r'measurement-category', measurements_api_views.CategoryViewSet, basename='measurement-category'
)
...
urlpatterns += [
    ...
    path(
        'api/v2/my-custom-exercise/search/',
        manager_api_views.MyCustomExerciseSearchView.as_view(),
        name='my-custom-exercise-search',
    ),
    # API auth
    path(
    ...
    path(
        'api/v2/schema/ui', SpectacularSwaggerView.as_view(url_name='schema'), name='api-swagger-ui'
    ),
