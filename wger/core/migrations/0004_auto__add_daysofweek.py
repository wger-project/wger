...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Renaming table 'DaysOfWeek'
        db.rename_table('manager_daysofweek', 'core_daysofweek')

        if not db.dry_run:
            # For permissions to work properly after migrating
            orm['contenttypes.contenttype'].objects.filter(app_label='manager', model='daysofweek').update(app_label='core')

    def backwards(self, orm):
        # Renaming table 'DaysOfWeek'
        db.rename_table('core_daysofweek', 'manager_daysofweek')

        if not db.dry_run:
            # For permissions to work properly after migrating
            orm['contenttypes.contenttype'].objects.filter(app_label='core', model='daysofweek').update(app_label='manager')
