...
def set_unit_types(apps: StateApps, schema_editor):
    """
    ...
    if RepetitionUnit.objects.filter(id=8).exists():
        logger.warning('RepetitionUnit with ID=8 already exists, skipping creation of "meters"')
    else:
        # New
        RepetitionUnit.objects.create(id=8, name='Meters', unit_type='DISTANCE', multiplier=1)
