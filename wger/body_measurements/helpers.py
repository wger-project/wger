...
def parse_body_measurements_csv(request, cleaned_data):
    ...
    parsed_csv = csv.reader(six.StringIO(cleaned_data['csv_input']),
                            dialect)
    distinct_body_measurements_entries = []
    entry_dates = set()
    ...
    for row in parsed_csv:
        try:
            ...
            parsed_body_measurements = decimal.Decimal(row[1].replace(',', '.'))
            duplicate_date_in_db = BodyMeasurementsEntry.objects.filter(date=parsed_date,
                                                              user=request.user).exists()
            # within the list there are no duplicate dates
            unique_among_csv = parsed_date not in entry_dates

            # there is no existing body measurements entry in the database for that date
            unique_in_db = not duplicate_date_in_db

            if unique_among_csv and unique_in_db and parsed_body_measurements:
                distinct_body_measurements_entries.append((parsed_date, parsed_body_measurements))
    ...
    # Create the valid body measurements entries
    for date, chest, hips, waist in distinct_body_measurements_entries:
        body_measurements_list.append(BodyMeasurementsEntry(date=date,
                                       chest=chest,
                                       hips=hips, 
                                       waist=waist,
                                       user=request.user))

    return (body_measurements_list, error_list)
