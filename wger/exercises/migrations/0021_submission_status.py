...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        '''
        Map the old submission status to the easier, more simple new ones:

        EXERCISE_STATUS_ADMIN ('4') and EXERCISE_STATUS_SYSTEM ('5') are removed
        and are just mapped to STATUS_ACCEPTED ('2')
        '''

        for exercise in orm.Exercise.objects.all():
            if exercise.status in ('4', '5'):
                exercise.status = '2'
                exercise.save()
