...
from wger.nutrition.gap_analyzer import analyze_nutrition_gap
from wger.nutrition.llm_coach import generate_coaching_message
...
class NutritionPlanViewSet(viewsets.ModelViewSet):
    ...
    @action(detail=True, url_path='smart-nutrition-gap')
    def smart_nutrition_gap(self, request, pk):
        """
        Return daily nutrition gaps and rule-based ingredient suggestions.
        """
        plan = self.get_object()
        date = self._get_gap_analysis_date()
        max_suggestions = self._get_max_suggestions_per_nutrient()

        analysis = analyze_nutrition_gap(plan, date=date)
        suggestions = suggest_ingredients_for_gaps(
            analysis,
            max_suggestions_per_nutrient=max_suggestions,
        )
        coaching_message = None

        if self._include_coaching_message():
            coaching_message = generate_coaching_message(analysis, suggestions)

        return Response(
            {
                'plan_id': plan.id,
                'date_analyzed': analysis.date,
                'consumed_totals': analysis.consumed.to_dict,
                'nutrient_gaps': {
                    nutrient: gap.to_dict for nutrient, gap in analysis.gaps.items()
                },
                'nutrient_statuses': {
                    nutrient: gap.status for nutrient, gap in analysis.gaps.items()
                },
                'goal_sources': {
                    nutrient: gap.goal_source for nutrient, gap in analysis.gaps.items()
                },
                'missing_goals': analysis.missing_goals,
                'ingredient_suggestions': suggestions.to_dict['suggestions'],
                'skipped_nutrients': suggestions.skipped_nutrients,
                'coaching_message': coaching_message,
            }
        )

    def _get_gap_analysis_date(self):
        """
        Parse an optional YYYY-MM-DD date query parameter for gap analysis.
        """
    ...
    def _get_max_suggestions_per_nutrient(self):
        """
        Parse an optional per-nutrient suggestion limit.
        """
        max_suggestions = self.request.query_params.get('max_suggestions', 5)

        try:
            max_suggestions = int(max_suggestions)
        except (TypeError, ValueError):
            raise ValidationError({'max_suggestions': 'Use a positive integer.'})
    ...
    def _include_coaching_message(self):
        """
        Return whether the optional OpenAI coaching layer should be called.
        """
        return self.request.query_params.get('include_coaching', '').lower() in (
            '1',
            'true',
            'yes',
        )
